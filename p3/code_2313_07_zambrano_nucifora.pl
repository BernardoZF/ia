...
segundo_penultimo(L, X, Y) :-
    length(L, N), N < 2, 
    write_log('Error 2.1 Longitud'), !, fail;
    penultimo(Y, L),
    segundo(X, L).
...
sublista_rec([X|L], 1, Mayor, Subl):-  
   Ma is Mayor-1, 
   sublista_rec(L, 1, Ma, SL),
   Subl = [X|SL].

sublista_rec([_|L], Menor, Mayor, SL):- 
   Ma is Mayor-1,
   Me is Menor-1,
   sublista_rec(L, Me, Ma, SL).

comp_elemento(L, E) :- member(E, L).
comp_elemento(_, _) :- write_log('ERROR 3.1 Elemento'), !, fail.
...
espacio_lineal_rec(_, Menor, Mayor, []) :- 
   Mayor < Menor, !.
espacio_lineal_rec(Npaso, Menor, Mayor, [Menor|R]) :- 
   S is Menor + Npaso,
   espacio_lineal_rec(Npaso, S, Mayor, R).
...
pruebaDivergencias(D1, _) :-
   constante(D1, R), R \== 1.0, write_log('ERROR 6.2. Divergencia KL definida solo para distribuciones.'), !, fail.
pruebaDivergencias(_, D2) :-
   constante(D2, R), R \== 1.0, write_log('ERROR 6.2. Divergencia KL definida solo para distribuciones.'), !, fail.
pruebaDivergencias(_, _) :- true.

divergencia_kl_rec([], [], 0) :-!.
divergencia_kl_rec([N|_], _, _) :- 
   N =< 0, write_log('ERROR 6.1. Divergencia KL no definida.'), !, fail.

divergencia_kl_rec(_, [N|_], _) :- 
   N =< 0, write_log('ERROR 6.1. Divergencia KL no definida.'), !, fail.

divergencia_kl_rec([N1|D1], [N2|D2], KL) :-
   divergencia_kl_rec(D1, D2, N),
   KL is N1*log(N1/N2)+N.

divergencia_kl(D1, D2, KL) :-
   pruebaDivergencias(D1,D2),
   divergencia_kl_rec(D1, D2, KL).
...
producto_kronecker(Matriz_A, Matriz_B, Matriz_bloques) :- 
   producto_kronecker_recA(Matriz_A, Matriz_B, Matriz_bloques).
   
producto_kronecker_recA([], _, []):-!.
producto_kronecker_recA([FA|MatA], MatB, [R1|R]) :-
   producto_kronecker_recA(MatA, MatB, R),
   producto_kronecker_recB(FA, MatB, R1).

producto_kronecker_recB([], _, []):-!.
producto_kronecker_recB([XA|_], _, _):-
   XA < 0, write_log('ERROR 7.1. Elemento menor que cero.'), !, fail.
producto_kronecker_recB([XA|FA], MatB, [R1|R]):-
   producto_kronecker_recB(FA, MatB, R),
   producto_kronecker_recC(XA, MatB, R1).

producto_kronecker_recC(_, [], []):-!.
producto_kronecker_recC(XA, [FB|MatB], [R1|R]):-
   producto_kronecker_recC(XA, MatB, R),
   producto_kronecker_recD(XA, FB, R1).

producto_kronecker_recD(_, [], []):-!.
producto_kronecker_recD(_, [XB|_], _):-
   XB < 0, write_log('ERROR 7.1. Elemento menor que cero.'), !, fail.

producto_kronecker_recD(XA, [XB|FB], [N|R]):-
   producto_kronecker_recD(XA, FB, R),
   N is XA * XB.
...
distancia_euclidea_rec([X1|R1], [X2|R2], D) :- 
   distancia_euclidea_rec(R1, R2, Sum),
   D is ((X1*X1) + (X2*X2) - (2*X1*X2) + Sum).

distancia_euclidea(X1, X2, D) :-
   distancia_euclidea_rec(X1, X2, R), D is sqrt(R).
...
calcular_distancias(X_entrenamiento, X_test, Matriz_resultados) :-
   calcular_distancias_recA(X_entrenamiento, X_test, Matriz_resultados).

calcular_distancias_recA(_, [], []) :-!.
calcular_distancias_recA(M1, [P|M2], [R|M3]) :- 
   calcular_distancias_recA(M1, M2, M3),
   calcular_distancias_recB(M1, P, R).
   

calcular_distancias_recB([], _, []) :-!.
calcular_distancias_recB([X|M1], Y, [D|R1]) :- 
   calcular_distancias_recB(M1, Y, R1),
...
predecir_etiquetas(Y_entrenamiento, K, [P|M_res], [R|Y_test]) :-
   predecir_etiquetas(Y_entrenamiento, K, M_res, Y_test),
   predecir_etiqueta(Y_entrenamiento, K, P, R).
...
unir_clave_valor([],[],[]):-!.
unir_clave_valor([X|L1], [Y|L2], [[Y,X]|R]) :- 
   unir_clave_valor(L1, L2, R).

calcular_K_etiquetas_mas_relevantes_rec(_, 0, []) :-!.
calcular_K_etiquetas_mas_relevantes_rec([[_,X]|L], K, [X|R]) :-
   K1 is K-1,
   calcular_K_etiquetas_mas_relevantes_rec(L, K1, R).
...
calcular_contadores_recA(_, [], []):-!.
calcular_contadores_recA(L, [E1|E], [[Sum,E1]|R]) :- 
   calcular_contadores_recB(L,E1,Sum),
   calcular_contadores_recA(L,E,R).


calcular_contadores_recB([], _, 0):-!.
calcular_contadores_recB([L1|L], E, R) :-
   L1 \== E,
   calcular_contadores_recB(L,E,R).

calcular_contadores_recB([L1|L], E, R) :-
   L1 == E,
   calcular_contadores_recB(L,E,R1),
   R is R1+1.

calcular_etiqueta_mas_relevante(K_etiquetas, Etiqueta) :- 
   sort(K_etiquetas, Etiquetas),
   calcular_contadores_recA(K_etiquetas, Etiquetas, Count),
